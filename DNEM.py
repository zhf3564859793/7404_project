...
import sys
sys.path.append('.')
from torchgeometric.read.ddi import read_ddi_data, read_ddi_600_800_data, read_ddi_800_1000_data

from torchgeometric.nn.models.model_1 import DDIDecoder, DDINet, GraphSAGE
from torchgeometric.nn.models.model_DDI_Energy_Net import DDI_Energy_Pooling, DDI_Energy_Net
import warnings
...
energy_phi_weight = args.energy_phi_weight
target = 0
dim = 100
#dim=113
threshold = 0.4
...
model_save_path = osp.join(osp.dirname(osp.realpath(
    __file__)), '../saved_models/', 'DNEM_800_1000_dataset_{}_seed_{}_ratio_{}_lr_{}_energy_phi_weight_{}_dropout_{}'.format(args.data_prefix, args.seed, args.data_ratio,args.lr,args.energy_phi_weight,dropout) + str('_no_ce_loss' if args.no_ce_loss else ''))
...
dataset = read_ddi_800_1000_data(args.data_dir, args.data_prefix,
                        random_seed=args.seed, data_ratio=args.data_ratio)
dataset.x=torch.tensor(dataset.x, dtype=torch.float32)
dataset.edge_attr=torch.tensor(dataset.edge_attr,dtype=torch.float32) 
...
#x1=np.zeros((1861,63))
x1=torch.tensor(x1)
dataset.x=torch.cat([dataset.x,x1],1)
print('合并后',dataset.x.shape)
dataset.x=torch.tensor(dataset.x, dtype=torch.float32)
...
#encoder1中的参数如下：input_feature_dim=50，num_types=113，dim=100
#encoder1 = DDIEncoder(input_feature_dim=dataset.x.size()[1], num_types=num_types, dim=dim)

encoder1=GraphSAGE(dataset.x.size()[1],200,100,2,dropout)

print('节点特征维度:',dataset.x.size()[1])
decoder1 = DDIDecoder(num_types, dim)
decoder2 = DDIDecoder(num_types, dim)

#encoder2 = DDIEncoder(input_feature_dim=dataset.x.size()[1], num_types=num_types, dim=dim)

encoder2=GraphSAGE(dataset.x.size()[1],200,100,2,dropout)
...
def train_max(epoch):  # max phi
    ...
    h = ddi_model.encode(
        data.x, data.edge_index, data.edge_attr)#由编码器encode将输入节点的特征，训练集中的边序号（实际由边端点序号索引而得到），边类型进行编码，得到训练集的节点最终的embedding表示
    pred = ddi_model.decoder(h, data.edge_index)#得到的节点embedding，再与边序号共同作为解码器的输入，得到训练集边类型的预测值pred
    loss_max = F.l1_loss(pred, data.edge_attr)
    ce_loss = F.binary_cross_entropy(pred, data.edge_attr)
    energy_phi = ddi_energy_model(
        data.x, data.edge_index, pred)#计算整个训练集所预测的能量值
    energy_y = ddi_energy_model(
        data.x, data.edge_index, data.edge_attr)#计算整个训练集真实的能量值

    # loss_max = ce_loss - F.relu(loss_max - energy_phi + energy_y)
    #

    h2 = ddi_model.encode(
        data.x, data.edge_index, data.edge_attr)#训练集节点的embedding表示
    pred2 = ddi_model.decoder2(h2, dataset.edge_index)#利用decoder2，将由训练集数据所获得的节点embedding和整个数据集的边序号作为输入，获得整个数据集的边类型预测值
    energy_psi = ddi_energy_model(dataset.x, dataset.edge_index, pred2)#在整个数据集上计算所预测的能量值

    pred3 = ddi_model.decoder2(h2, data.edge_index)#利用decoder2，将由训练集数据所获得的节点embedding和训练集的边序号作为输入，获得训练集的边类型预测值
...
def train_min(epoch):  # min theta
    ...
    energy_phi = ddi_energy_model(
        data.x, data.edge_index, pred)#计算训练集上的预测能量
    energy_y = ddi_energy_model(
        data.x, data.edge_index, data.edge_attr)#计算训练集上的真实能量
    ...
    return loss_min.item()#从loss_min这个tensor类型转换为标量类型（数值型）
...
if args.only_test:
    ...
    correlation(test_dataset)
    pair_wise_correlation(test_dataset)
